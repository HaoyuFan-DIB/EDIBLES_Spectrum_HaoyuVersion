...
class GaussianLine_KnownWav(ArithmeticModel):
    ...
    def __init__(self, name='GaussianLine', lam_0=5000, v_max = 1000, b=1.5, N=999, f=999, N_mag=10, CD=0.1):
        self.N_mag = N_mag
...
class GaussianLine_Asymmetry(ArithmeticModel):
    def __init__(self, name='AsymmetricGaussianLine', lam_0=5000, b_left=1.5, b_right=1.5, N=999, f=999, N_mag=10, CD=0.1):
        self.N_mag = N_mag
        self.lam_0 = Parameter(name, 'lam_0', lam_0, frozen=False)
        self.b_left = Parameter(name, 'b_left', b_left, frozen=False, min=1e-12)
        self.b_right = Parameter(name, "b_right", b_right, frozen=False, min=1e-12)
        self.N = Parameter(name, 'N', N, frozen=True, hidden=True, min=0.0)
        self.f = Parameter(name, 'f', f, frozen=True, hidden=True, min=0.0)
        self.CD = Parameter(name, 'CD', CD, frozen=False, min=0.0)
        self.kernel_offset = Parameter(name, 'kernel_offset', 0, frozen=True, hidden=True)
        ...
        ArithmeticModel.__init__(self, name,
                                 (self.lam_0, self.b_left, self.b_right,
                                  self.N, self.f, self.CD, self.kernel_offset))

        if N != 999 and f != 999: self.__useNf__(N=N, f=f)

    def calc(self, pars, x, *args, **kwargs):
        lam_0, b_left, b_right, N, f, CD, kernel_offset = pars
        if N !=999 and f != 999:
            N = N * (10**self.N_mag)
            transmission_l = eMath.gaussianAbsorption(lam=x, lam_0=lam_0, b=b_left, N=N, f=f)
            transmission_r = eMath.gaussianAbsorption(lam=x, lam_0=lam_0, b=b_right, N=N, f=f)
        else:
            transmission_l = eMath.gaussianAbsorption(lam=x, lam_0=lam_0, b=b_left, CD=CD)
            transmission_r = eMath.gaussianAbsorption(lam=x, lam_0=lam_0, b=b_right, CD=CD)

        weight_l = np.zeros_like(x)
        weight_l[x <= lam_0] = 1
        transmission = transmission_l * weight_l + transmission_r * (1-weight_l)
        return transmission
...
class SplineContinuum(ArithmeticModel):
        ...
        if self.x_anchor is None:
            ...
            for i in range(1, len(y_sections), 2):
                ...
                if i == range(len(y_sections))[-1]:
                    ...
                    exec("self.y_{i}.min = 0.995*np.median(span)".format(i=y_idx))
                    exec("self.y_{i}.max = 1.005*np.median(span)".format(i=y_idx))
                else:
            ...
            for i, anchor in enumerate(self.x_anchor):
                ...
                exec("self.y_{i}.min = 0.995*np.mean([left, right])".format(i=i))
                exec("self.y_{i}.max = 1.005*np.mean([left, right])".format(i=i))
    ...
    def calc(self, pars, x, *args, **kwargs):
        ...
        self.x_anchor = x_points
        self.y_used = y_points
        spline = CubicSpline(x_points, y_points)
        return spline(x)

    def __str__(self):
        str_out = "Spline Continuum: {name}\n".format(name=self.name)
        str_out = str_out + "Anchor X:\n"
        str_out = str_out + str(self.x_anchor) + "\n"
        str_out = str_out + "Anchor Y:\n"
        str_out = str_out + str(self.y_used) + "\n"
        return str_out
...
class Instrumental(ArithmeticModel):

    def __init__(self):
        self.kernel = [0, 2, 0]

    def calc(self, pars, x, *args, **kwargs):
        convolve1d(x, self.kernel, output=x, mode='constant', cval=1.0)
        return x