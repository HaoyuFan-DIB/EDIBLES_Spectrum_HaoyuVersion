...
class SingleSpectrum:
    ...
    def __loadFits__(self, filename, name=None):
        ...
        if name is None:
            self.specname = spec_name.split("_")[0]
    ...
    def __loadASICII__(self, filename, name=None):
        ...
        if self.delimiter is None:
            f = open(os.path.join(Setting.datadir, filename))
            header = f.readline()
        ...
        if self.delimiter == " ":
            df = pd.read_csv(os.path.join(Setting.datadir, filename), delim_whitespace=True)
        else:
            df = pd.read_csv(os.path.join(Setting.datadir, filename), delimiter=delimiter)
...
class EdiblesSpectrum:
    ...
    def getKernel(self, resolution=120000, n_sigma=5, apply_mask=False, panels=None):
        panels = self.__parsePanelsInput__(panels)
    ...
    def fitModel(self, link_b=None, link_d=None, freeze_d=None, opt="NelderMead",
        ...
        data2fit = Data1D('data2fit', x, y, y/SNR)
        ...
        for i, panel in enumerate(panels):
            x_plot, y_plot, masked_plot = self.specdata[panel].getData(apply_mask=False)
            x_model, y_model, masked_model = self.specdata[panel].getData(apply_mask=apply_mask, x_out="wave")
            axs = fig.axes[i * 2]
            axs.plot(x_plot, y_plot, color="k")
            if apply_mask:
                axs.scatter(x_plot[masked_plot == 1], y_plot[masked_plot == 1], marker="x", color="red")
            n_offset = self.__getKernelOffset__(apply_mask=apply_mask, panels=panel)
            v_offset = (x_model[1] - x_model[0]) / np.median(x_model) * cst.c.to("km/s").value * n_offset
            model_plot_panel = self.model.compileModel(link_b=link_b, link_d=link_d,
                                    freeze_d=freeze_d, conv_correction=v_offset)
            axs.plot(x_model, model_plot_panel(x_model), color="red")
            axs.plot(x_model, self.model.continuum(x_model), linestyle='--', color="orange")
            axs.grid()
            ...
            # residual
            axs = fig.axes[i*2 + 1]
            axs.plot(x_model, (model_plot_panel(x_model) - y_model)/self.model.continuum(x_model), color='k')
            axs.plot(x_model, np.zeros_like(x_model), linestyle='--', color="orange")
            if self.specdata[panel].SNR is None: self.specdata[panel].estimateSNR()
            axs.plot(x_model, np.ones_like(x_model)/self.specdata[panel].SNR, linestyle='--', color="blue")
            axs.plot(x_model, -1 * np.ones_like(x_model)/self.specdata[panel].SNR, linestyle='--', color="blue")
            axs.grid()
            if i == n_rows - 1:
...
if __name__ == "__main__":
    ...
    l = 5109
    f=0.001
    N = 1.13 * (10**20) * W /(l**2 * f)

    N = 4 * 10**12
    W = N * l**2 * 0.001 / 1.13 / 10**20
