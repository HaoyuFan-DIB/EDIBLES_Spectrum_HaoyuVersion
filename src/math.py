...
def voigtMath(x, sigma, gamma):
    ...
    :param sigma: Gaussian factor for Voigt, see Overleaf documentation, in AA
    :type sigma: float
...
def voigtAbsorption(lam, lam_0, b, d, tau_0 = 1.0):
    """
    ...
    :type d: float
    :param tau_0: sum of optical depth, in AA, default to 1.0
    :type tau_0: float
    ...
    # convert b to sigma
    sigma = b * lam_0 / cst.c.to('km/s').value / np.sqrt(2)
    ...
    y = voigtMath(x, sigma, gamma)

    voigt_absorption = - y * tau_0
...
def voigtOpticalDepthAbsorption(lam, lam_0, b, d, tau_0=0.1, N=None, f=None):
    ...
    if (N is not None) and (f is not None):
        ...
        tau_0 = (
            ...
            * (cst.e.esu.value) ** 2
            * Nf
            * (1e-8 * lam_0) ** 2   # convert lambda from AA to cm
            / (cst.m_e.to("g").value * (cst.c.to("cm/s").value) ** 2)
        )  # cm
        # Convert cm to angstroms
        tau_0 *= 1e8

    voigt_opticaldepth = voigtAbsorption(lam, lam_0, b, d, tau_0 = tau_0)

    transmission = np.exp(voigt_opticaldepth)

    return transmission

def gaussianAbsorption(lam, lam_0, b, CD=0.1, N=None, f=None):
    """
    Returns a Gaussian profile absorption
    If N and F were given, the Gaussian profile will be calculated assuming linear section of the COG
    ...
    :param b: Gaussian standard deviation, in km/s
    :type b: float
    :param CD: Central depth of the Gaussian profile, overrode by Nf
    :type CD: float
    :param N: column density, in 10E12 cm^-2
    :type N: float
    :param f: oscillator strength
    :type f: float
